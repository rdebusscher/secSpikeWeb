...
import static org.omnifaces.security.jaspic.Utils.*;
...
public class OmniServerAuthContext implements ServerAuthContext {
    ...
    public static final String AUTH_METHOD_SESSION_NAME = "org.omnifaces.javax.security.jaspic.AuthMethod";
    public static final String REMEMBER_ME_SESSION_NAME = "org.omnifaces.javax.security.jaspic.RememberMe";
    ...
    public AuthStatus validateRequest(MessageInfo messageInfo, Subject clientSubject, Subject serviceSubject) throws AuthException {
        ...
        // FIXME For Web access (JSF) how can we define which URL are protected?
        HttpServletRequest servletRequest = (HttpServletRequest) messageInfo.getRequestMessage();
        String requestURI = servletRequest.getRequestURI();
        if (requestURI.startsWith(servletRequest.getContextPath())) {
            requestURI = requestURI.substring(servletRequest.getContextPath().length());
        }
        if (requestURI.startsWith("/pages/")) {
            messageInfo.getMap().put("javax.javax.security.auth.message.MessagePolicy.isMandatory", Boolean.TRUE.toString());
        }
        // End addition.

        AuthStatus status = doValidateRequest(messageInfo, clientSubject, serviceSubject);
        Jaspic.setLastStatus(servletRequest, status);
